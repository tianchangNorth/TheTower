...
Agent
  -> MCP tool call
  -> @the-tower/mcp-server
  -> /api/callbacks/tools/*
  -> WorkspaceFileService
  -> Thread.projectPath / WorkspaceResolver
  -> filesystem
```

关键点：

1. MCP server 不直接读写文件。
2. API 使用 `invocationId + callbackToken` 验证调用者。
3. API 确认 invocation 仍在 running。
4. API 通过 invocation 找到 thread，再通过 thread 找到 workspace。
5. 所有文件路径都相对当前 thread workspace 解析。
6. 绝对路径也必须落在当前 thread workspace 内。
7. symlink 必须按 realpath 校验，不能逃逸 workspace。
8. 文件工具写入前必须拒绝 `.git` 等敏感目录。
...
这里的 handler 不直接操作文件系统。文件工具 handler 调用 `CallbackClient`，由 API 侧完成真实 IO。
...
3. `read-only` 不暴露 `post_message` 和 `write_file`，除非后续增加 agent-key 等显式凭证模式。
...
3. `write_file` 是 non-destructive write，不标 destructive；它会覆盖目标文件，但仍限制在 workspace 内。后续如果新增 `delete_file` / `shell_exec`，才进入 destructive。
4. `openWorldHint` 第一版全部为 false，因为工具只访问 TheTower 本地 API 和 workspace。

### API 托管与猫咖本地工具的差异

猫咖 `file-tools.ts` 直接在 MCP 进程内用 `isPathAllowed` 操作文件。TheTower 不照搬这个执行位置，而是照搬它的边界思想：

| 维度 | 猫咖 | TheTower 第一版 |
| --- | --- | --- |
| 文件 IO 位置 | MCP server 进程 | API `WorkspaceFileService` |
| 路径根 | `ALLOWED_WORKSPACE_DIRS` + cat-cafe data dir | `Thread.projectPath` resolve 后的 `workingDirectory` |
| 凭证 | readonly env / agent-key / callback | invocation `callbackToken` |
| 工具 schema | MCP tool module | MCP tool module |
| 工具注册 | `server-toolsets.ts` | `server-toolsets.ts` |
| annotations | explicit table | explicit table |

这样既保留猫咖的可维护结构，又避免 MCP 进程本身成为文件权限边界。
...
`ALLOWED_WORKSPACE_DIRS` 可以继续注入作为 defense-in-depth，但不能作为唯一权限来源。真实权限以 API 侧 WorkspaceResolver 为准。
...
这些都应归入后续 “TheTower 托管命令执行 / 审批” 阶段。
...
8. Claude runner `--allowedTools` 加入新增 MCP 工具。
9. Codex prompt / MCP config 说明新增工具。
10. 增加文件工具审计事件。
11. 增加 API 单元测试覆盖路径边界。
12. 增加 MCP server 测试覆盖工具注册、profile 过滤、annotations 和 callback 转发。
