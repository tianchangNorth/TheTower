...
export function buildCodexPrompt(input: AgentRunInput, apiBaseUrl = process.env.THE_TOWER_API_URL ?? "http://127.0.0.1:3001"): string {
  return [
    ...
    "当当前 routeMode 是 fanout 或 parallel 时，你只完成自己的部分；不要在最终回复里 @ 当前 worklist 中等待执行的 Agent。",
    "",
    "### HTTP 回调（异步）",
    ...
    "targetAgents 是结构化路由目标；routeMode 可为 single、serial、fanout、parallel。fanout/parallel 中普通文本 @ 默认不会继续路由，显式 targetAgents 仍会路由。",
