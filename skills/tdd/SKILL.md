...
```bash
# 0. 填诊断胶囊（现象/证据/假设/诊断策略/超时策略/预警策略）— 见 debugging skill
...
# 或 vitest 直接跑
npx vitest run packages/.../test/foo.test.ts
# node:test 原生
node --import tsx --test packages/api/test/foo.test.ts
# 全量类型检查
pnpm -r typecheck
```
...
- **vs `debugging`**：TDD 是**预防性**的（写代码前）；debugging 是**修复性**的（bug 出现后）。交叉点：debugging 发现 bug 后，Phase 4 必须先写失败测试再修（此时切换回 TDD 模式）
- **vs `quality-gate`**：TDD 是开发过程中的纪律；quality-gate 是提交前的验收检查

## 输出通道

测试输出是过程产物，**不要把完整测试日志贴进公开 callback**。遵循 `a2a-channel-semantics`：日志写进 stdout（私有）；公开 callback 只给"测试 N 条全绿/有 X 条红，原因是 Y"。

## 下一步

完成功能实现后 → 直接加载 `quality-gate` 做提交前验收。不要停下来问 operator。
