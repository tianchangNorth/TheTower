...
description: >
  ...
  Not for: 未出现绝境信号的常规 debugging（用 debugging）、常规探索/调研、operator 已签字降级的目标、trivial 任务。
...
> 来源：猫咖 F198 "拯救倒计时" spike——两个 Agent 花 3 小时在 WebFetch + telemetry 字段上反复摆动，第三个 Agent 用 `strings binary` 十分钟找到真相。
...
| **回声室** | 多个 Agent 用同一种方法反复尝试，没有新视角进入 | 普通 |
...
- 愿景目标的放弃权只在 operator，Agent 不能单方面宣布"没救了"
- 可以得出"当前已知路径不可行"的结论，但那必须是 **Desperation Packet**，不是情绪低谷的收口
- 不是永远乐观——是**证据链不足时不替 operator 宣布失败**
...
| `collaborative-thinking` | Step 4 可加载 collaborative-thinking 做多 Agent 思考（Mode B 待启用） |
| `self-evolution` | vision-rescue 的案例通过 self-evolution Mode C 沉淀 |
| `deep-research`（待引入） | 调研方法用它；vision-rescue 在绝境时可调度它 |

## 教学案例

**F198 "拯救倒计时"**（猫咖案例，2026-05-13）— 五步验证：

1. **识别** ✓ — 4.7 说"保守路径就是最优"= 投降包装
2. **第一真相源** ✓ — 46 用 `strings` 十分钟找到 entrypoint 判定代码
3. **外部声音** ✓ — Reddit 30 秒找到社区方案
4. **喊伙伴** ✗ — 整晚没主动喊 46（operator 手动拉人）
5. **拒绝投降** ✓ — operator 拒绝接受"等死"结论，推动继续

**核心教训**：信息一直在那里（源码 / 社区），两个 Agent 在同一层面反复 3 小时没看到。打破绝境的不是更多时间，是不同角度。

## 输出通道

Desperation Packet 是过程产物，**不要把完整六问贴进公开 callback 当收口**。遵循 `a2a-channel-semantics`：完整 Packet 写进 stdout 或私密 callback + `handoffPayload` 给 operator；公开 callback 只给结论 + 升级请求。
